classdef occupancy_grid_map_continuous_CSM < handle
    ...
    methods
        function obj = occupancy_grid_map_continuous_CSM(pose, scan)
            ...
            % -----------------------------------------------
            init_prior = 0.001;
            obj.map.alpha = repmat(init_prior, obj.map.size, 1);
            obj.map.beta = repmat(init_prior, obj.map.size, 1);
        ...
        function continuous_counting_sensor_model(obj, z, i)
            ...
            if obj.m_i.range > min(obj.z_max, z(k,1) + obj.alpha/2) || bearing_min > obj.beta/2
                ...
                if d_end < obj.l && z(k,1) < obj.z_max
                    obj.map.alpha(i) = obj.map.alpha(i) + obj.kernel(d_end);
                end
                ...
                if d_proj < obj.l && x_proj < z(k,1)
                    obj.map.beta(i) = obj.map.beta(i) + obj.kernel(d_proj);
                end
            end
        end
